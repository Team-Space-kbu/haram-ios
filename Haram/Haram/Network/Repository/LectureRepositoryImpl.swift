...
extension LectureRepositoryImpl: LectureRepository {
  func inquireEmptyClassBuilding() -> RxSwift.Single<[String]> {
    service.request(router: LectureRouter.inquireEmptyClassBuilding, type: [String].self)
  }
  
  func inquireEmptyClassList(classRoom: String) -> RxSwift.Single<[String]> {
    service.request(router: LectureRouter.inquireEmptyClassList(classRoom), type: [String].self)
  }
  
  func inquireEmptyClassDetail(classRoom: String) -> RxSwift.Single<[InquireEmptyClassDetailResponse]> {
    service.request(router: LectureRouter.inquireEmptyClassDetail(classRoom), type: [InquireEmptyClassDetailResponse].self)
  }
  
  func inquireCoursePlanList() -> RxSwift.Single<[InquireCoursePlanListResponse]> {
    service.request(router: LectureRouter.inquireCoursePlanList, type: [InquireCoursePlanListResponse].self)
  }
  
  func inquireCoursePlanDetail(course: String) -> RxSwift.Single<[InquireEmptyClassDetailResponse]> {
    service.request(router: LectureRouter.inquireCoursePlanDetail(course), type: [InquireEmptyClassDetailResponse].self)
  }
}
