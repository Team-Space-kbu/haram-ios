...
final class CoursePlanViewModel: ViewModelType {
  ...
  struct Dependency {
    ...
    let coordinator: CoursePlanCoordinator
  }
  ...
  func transform(input: Input) -> Output {
    ...
    input.didTapMajorCell
      .withLatestFrom(output.majorList) { $1[$0.row].courseKey }
      .subscribe(with: self) { owner, course in
        owner.dependency.coordinator.showLectureInfoViewController(course: course)
      }
      .disposed(by: disposeBag)
