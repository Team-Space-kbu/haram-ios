...
final class CheckReservationViewModel: ViewModelType {
  ...
  struct Output {
    let rothemReservationInfoViewRelay  = PublishRelay<RothemReservationInfoViewModel>()
    let successCancelReservationSubject = PublishSubject<Void>()
  ...
  func inquireRothemReservationInfo(output: Output) {
    ...
    inquireRothemReservationInfo
      .subscribe(with: self, onSuccess: { owner, response in
        ...
        output.rothemReservationInfoViewRelay.accept(model)
