...
final class ChapelViewModel: ViewModelType {
  ...
  struct Output {
    let chapelListModelRelay   = PublishRelay<[ChapelCollectionViewCellModel]>()
    let chapelDetailModelRelay = PublishRelay<[ChapelDetailInfoViewModel]>()
    let chapelHeaderModelRelay = PublishRelay<ChapelCollectionHeaderViewModel?>()
    let isLoadingSubject       = BehaviorRelay<Bool>(value: true)
  ...
  func inquireChapelInfo(output: Output) {
    dependency.intranetRepository.inquireChapelInfo()
      .subscribe(with: self, onSuccess: { owner, response in
        ...
        output.isLoadingSubject.accept(false)
  ...
  func inquireChapelDetail(output: Output) {
    dependency.intranetRepository.inquireChapelDetail()
      .subscribe(with: self, onSuccess: { owner, response in
        ...
        output.chapelListModelRelay.accept(chapelListModel)
        output.isLoadingSubject.accept(false)
      }, onFailure: { owner, error in
        guard let error = error as? HaramError else { return }
        output.errorMessageRelay.accept(error)
