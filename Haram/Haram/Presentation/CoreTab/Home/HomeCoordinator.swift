...
extension HomeCoordinator {
  ...
  func showEmptyClassViewController() {
    let coordinator = CampusBuildingListCoordinator(navigationController: self.navigationController)
    coordinator.start()
  ...
  func showNoticeViewController() {
    let coordinator = NoticeListCoordinator(navigationController: self.navigationController)
    coordinator.start()
    self.childCoordinators.append(coordinator)
  }
  
  func showLibraryViewController() {
    let coordinator = BookListCoordinator(navigationController: self.navigationController)
    coordinator.start()
    self.childCoordinators.append(coordinator)
  }
  
  func showCoursePlanViewController() {
    let coordinator = DepartmentListCoordinator(navigationController: self.navigationController)
    coordinator.start()
    self.childCoordinators.append(coordinator)
  }
  
  func showAffiliatedViewController() {
    let coordinator = AffiliatedListCoordinator(navigationController: self.navigationController)
    coordinator.start()
    self.childCoordinators.append(coordinator)
  }
  ...
  func showRothemViewController() {
    let coordinator = RothemRoomListCoordinator(navigationController: self.navigationController)
    coordinator.start()
    self.childCoordinators.append(coordinator)
  }
