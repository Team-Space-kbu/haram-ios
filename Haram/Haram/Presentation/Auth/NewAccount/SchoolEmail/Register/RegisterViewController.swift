    ...
    output.errorMessageRelay
      .subscribe(with: self) { owner, error in
        if error == .noEqualPassword {
          ...
          AlertManager.showAlert(on: self.navigationController, message: .custom("해당 아이디는 이미 사용중입니다\n다른 아이디로 수정해주세요."))
        } else {
          AlertManager.showAlert(on: self.navigationController, message: .custom("서버측에서 알 수 없는 에러가 발생하였습니다\n다음에 다시 시도해주세요."))
        }
      }
